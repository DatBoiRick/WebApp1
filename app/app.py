...
def form_edit_get(city_id):
    user = {'username': 'Ricardo'}
    cursor = mysql.get_db().cursor()
    cursor.execute('SELECT * FROM mlb_players WHERE id=%s', city_id)
    result = cursor.fetchall()
...
def form_update_post(mlb_id):
    ...
    sql_update_query = """UPDATE mlb_players t SET t.fldName = %s, t.fldTeam = %s, t.fldPosition = %s, t.fldHeight_inches = 
...
def form_insert_get():
    user = {'username': 'Ricardo'}
    return render_template('new.html', title='New Player Stat', user=user )
...
def form_insert_post():
    ...
    sql_insert_query = """INSERT INTO mlb_players (fldName,fldTeam,fldPosition,fldHeight_inches,fldWeight_lbs,fldAge) VALUES (%s, %s,%s, %s,%s, %s) """
...
def api_edit(mlb_id) -> str:
    ...
    inputData = (content['fldName'], content['fldTeam'], content['fldPosition'],
                 content['fldHeight_inches'], content['fldHeight_inches'],
